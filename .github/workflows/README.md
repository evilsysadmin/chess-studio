...
| `staging-pages.yml` | Activo · nombre legacy | Despliega el SHA aprobado por CI en Render staging + Cloudflare Pages staging y acredita backend/frontend con smoke real. **No es GitHub Pages**; el nombre del fichero es histórico. |
...
`staging-pages.yml` conserva un **nombre** legacy, no una implementación legacy. Renombrarlo puede hacerse cuando compense el ruido de historial, pero borrarlo sería romper el despliegue real de staging.
...
Resultado de la auditoría 2026-09-02: se encontró y retiró un artefacto operativo realmente obsoleto (`gh-pages`). No se encontró otro workflow cuya eliminación sea segura sin perder un gate, una operación manual útil, observabilidad o preparación explícita de infraestructura.
