...
test('War Room · el bocadillo de Matthias sigue al rey si cambia de casilla', async ({ page }) => {
  ...
  const specialRules = quickDialog.locator('details.friendly-subdisclosure').filter({ hasText: 'Sudden Death' }).first();
  if (!(await specialRules.evaluate((node) => node.open))) await specialRules.locator(':scope > summary').click();
  await specialRules.getByRole('checkbox', { name: /Sudden Death/ }).check();
