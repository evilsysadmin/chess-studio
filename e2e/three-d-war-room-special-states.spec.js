...
test('War Room parity · promoción 3D abre selector y conserva la pieza elegida al volver a 2D', async ({ page }) => {
  ...
  const knightButton = promotionDialog.getByRole('button', { name: 'Caballo', exact: true });
  await expect(knightButton).toBeVisible();
  await expect(knightButton).toBeEnabled();
  try {
    await knightButton.click({ timeout: 12_000 });
  } catch {
    await knightButton.evaluate((element) => element.click());
  }
