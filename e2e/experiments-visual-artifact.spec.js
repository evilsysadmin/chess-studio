...
test('Experimentos · landing visual desktop + Android', async ({ browser }) => {
  test.skip(!scopeEnabled('landing'), 'Experiments landing is outside this PR visual scope');
...
test('Chronicles · gameplay visual desktop + Android', async ({ browser }) => {
  test.skip(!scopeEnabled('chronicles'), 'Chronicles is outside this PR visual scope');
...
test('Pawn Slug · ready/live visual desktop + Android', async ({ browser }) => {
  test.skip(!scopeEnabled('pawnslug'), 'Pawn Slug is outside this PR visual scope');
  test.setTimeout(120_000);
  await mkdir(ARTIFACT_DIR, { recursive: true });
