...
test('Home · la guía inicial no bloquea, recuerda el cierre y puede reabrirse', async ({ page }) => {
  ...
  await expect(guide).toBeVisible();
  await expect(guide.getByRole('button', { name: 'Jugar primer rival', exact: true })).toBeVisible();
});
...
test('Home · cuenta y cierre de sesión son acciones accesibles', async ({ page }) => {
  ...
  await expect(page.getByRole('menuitem', { name: /Novedades/ })).toBeVisible();
  ...
  await page.getByRole('menuitem', { name: /Novedades/ }).click();
  await expect(page.getByRole('dialog', { name: 'Qué ha cambiado para ti' })).toBeVisible();
