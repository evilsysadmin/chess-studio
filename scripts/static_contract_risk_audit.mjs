...
const frontendModules = discoverFrontendModules(frontendSrc);
...
const boundaryViolations = legacyBoundaries.flatMap((boundary) => {
  const ownerPath = path.resolve(frontendSrc, boundary.owner);
  const consumers = frontendModules.filter((file) => {
    if (path.resolve(file) === ownerPath) return false;
    return fs.readFileSync(file, 'utf8').includes(boundary.legacyModule);
  });
  return consumers.map((file) => ({ ...boundary, file }));
});
...
if (boundaryViolations.length) {
  for (const violation of boundaryViolations) {
    console.error(`FAIL: ${violation.label} bypassed by ${relative(violation.file)}`);
    console.error(`Importa desde frontend/src/${violation.owner}; sólo esa fachada puede depender de ${violation.legacyModule}.`);
  }
  process.exit(1);
}
