...
if (checkCiWiring) {
  ...
  for (const required of [
    ...
    '# Production service lanes: backend and Worker start in parallel from the immutable gate.',
    'name: Production · frontend build',
    'production_deploy_target.py resolve',
    'DEPLOY_TARGET: ${{ needs.gate.outputs.deploy_target }}',
    "if: env.DEPLOY_TARGET == 'render'",
    'python3 scripts/render_production_deploy.py --sha "$DEPLOY_SHA"',
    'python3 scripts/oci_production_tunnel.py render --sha "$DEPLOY_SHA"',
    "if: env.DEPLOY_TARGET == 'oci'",
    'python3 scripts/oci_production_runtime.py bootstrap',
    'python3 scripts/oci_production_runtime.py sync-current',
    'oci_release_deploy.py deploy --target production --repo-ref "$DEPLOY_SHA"',
    'oci_production_tunnel.py activate --sha "$DEPLOY_SHA"',
    'name: Production · Cloudflare Pages',
    'needs: [gate, cloudflare, backend, pages_build]',
...
if (!playwrightConfig.includes("process.env.PLAYWRIGHT_TRACE === '1'") || !playwrightConfig.includes("trace: traceDiagnostics ? 'retain-on-failure' : 'off'")) fail('Playwright trace debe ser opt-in diagnóstico; no se graba en cada test verde');
...
if (!e2eSource.includes("getByRole('region', { name: 'Modos principales', exact: true })")) {
  fail('El smoke responsive de Home debe anclarse a la región estable "Modos principales"');
}
