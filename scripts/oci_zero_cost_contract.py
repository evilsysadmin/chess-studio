...
expansion into obviously billable resource families.
"""
...
OCI_ROOTS = (ROOT / "infra" / "oci" / "bootstrap", ROOT / "infra" / "oci" / "staging")
...
FORBIDDEN_MARKERS = {
    "oci_core_image": "custom Compute images are outside the zero-cost contract",
    "oci_containerengine_": "managed OKE is not part of the single-A1 K3s lab",
    "oci_core_volume": "extra Block Volumes require a separate storage-budget review",
    "oci_core_boot_volume": "standalone boot-volume resources require a separate storage-budget review",
    "oci_database_": "managed database resources are outside the OCI staging lab contract",
    "oci_file_storage_": "File Storage is outside the OCI staging lab contract",
    "oci_functions_": "OCI Functions are outside the OCI staging lab contract",
    "oci_nat_gateway": "NAT Gateway is unnecessary for the current public-subnet lab",
    "oci_kms_vault.*VIRTUAL_PRIVATE": "Virtual Private Vault is explicitly outside Always Free",
}


def terraform_text() -> tuple[str, list[Path]]:
    files: list[Path] = []
    chunks: list[str] = []
    for root in OCI_ROOTS:
...
def validate(root: Path = ROOT) -> None:
    ...
    for marker, reason in FORBIDDEN_MARKERS.items():
        if ".*" in marker:
            if re.search(marker, text, re.DOTALL):
                raise SystemExit(f"OCI zero-cost contract: FAIL · {reason}")
        elif marker in text:
            raise SystemExit(f"OCI zero-cost contract: FAIL · {reason}")
