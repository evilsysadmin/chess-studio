...
def add_gothic_canon_v2(static, mats):
    ...
    # Keep the campaign panel graphic and line-led. Broad territory blobs were
    # rejected in visual review because they read as pasted ovals rather than map relief.
    ...
    cz = 6.28
    # Keep the chandelier centered inside the *actual* free bay between the
    # crest and campaign display. The previous compact pass still left almost
    # no negative space beside the crest, so reduce the radial footprint and
    # nudge the fixture right while preserving its six-candle identity.
    chandelier_x = 2.20
    chandelier_y = 2.85
    torus("WR_CANON_chandelier_ring", (chandelier_x, chandelier_y, cz), 0.52, 0.034, mats["brass"], static)
    ...
    for stay_index, angle in enumerate((math.pi / 2, math.pi / 2 + math.tau / 3, math.pi / 2 + 2 * math.tau / 3)):
        stay_start = Vector((
            chandelier_x + math.cos(angle) * 0.43,
            chandelier_y + math.sin(angle) * 0.33,
            cz + 0.03,
        ))
    ...
    for index in range(6):
        ...
        x = chandelier_x + math.cos(angle) * 0.43
        y = chandelier_y + math.sin(angle) * 0.33
