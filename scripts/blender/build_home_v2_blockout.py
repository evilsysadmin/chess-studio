    ...
    for obj in list(bpy.data.objects):
        if ("fireplace_left" in obj.name) and obj.type != "LIGHT":
            ...
            if "_flame_" in obj.name or "_flame_hot_" in obj.name:
                # The canonical v36 overlay owns the visible fire silhouette.
                # Keep the legacy flames as a buried glow only so they cannot
                # read as a second row of vertical spikes.
                obj.scale.x *= 0.46
                obj.scale.z *= 0.32
                obj.location.y += 0.24
