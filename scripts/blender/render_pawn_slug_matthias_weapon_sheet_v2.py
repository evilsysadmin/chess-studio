...
def materials():
    return {
        'skin': canonical.mat('v2_skin', (0.72, 0.50, 0.36, 1), 0.0, 0.62),
        'uniform': canonical.mat('v2_uniform', (0.035, 0.045, 0.060, 1), 0.03, 0.48),
        'uniform2': canonical.mat('v2_uniform2', (0.075, 0.090, 0.115, 1), 0.05, 0.42),
        'armor': canonical.mat('v2_armor', (0.14, 0.17, 0.20, 1), 0.24, 0.30),
        ...
        'gunmetal': canonical.mat('v2_gunmetal', (0.18, 0.22, 0.27, 1), 0.78, 0.20),
        'dark': canonical.mat('v2_weapon_dark', (0.035, 0.045, 0.055, 1), 0.36, 0.28),
        'polymer': canonical.mat('v2_polymer', (0.070, 0.080, 0.095, 1), 0.08, 0.38),
        'olive': canonical.mat('v2_olive', (0.24, 0.30, 0.15, 1), 0.26, 0.39),
...
def cube(name, loc, dims, material, rot=(0.0, 0.0, 0.0), parent=None):
    ...
    return canonical.cube(name, loc, tuple(dims), material, rot=rot, parent=parent)
...
def build_matthias(origin, action, frame, count, weapon, m):
    ...
    cube('v2_pelvis', canonical.xz(origin, 0.10, 0.79 + body_z, 0.02), (0.54, 0.38, 0.30), m['uniform'], parent=base)
    cube('v2_torso', canonical.xz(origin, 0.13, 1.19 + body_z, 0.02), (0.66, 0.42, 0.66), m['uniform'], rot=(0, p['lean'], 0), parent=base)
    cube('v2_vest', canonical.xz(origin, 0.04, 1.20 + body_z, -0.24), (0.50, 0.10, 0.50), m['armor'], parent=base)
    cube('v2_pouch', canonical.xz(origin, 0.25, 0.98 + body_z, -0.29), (0.20, 0.11, 0.18), m['armor'], parent=base)
    canonical.sphere('v2_face', canonical.xz(origin, 0.16, 1.70 + body_z, -0.20), (0.21, 0.16, 0.20), m['skin'], segments=18, rings=9, parent=base)
    canonical.sphere('v2_helmet', canonical.xz(origin, 0.18, 1.87 + body_z, -0.02), (0.29, 0.22, 0.18), m['helmet'], segments=18, rings=9, parent=base)
    cube('v2_helmet_brim', canonical.xz(origin, -0.01, 1.79 + body_z, -0.22), (0.34, 0.10, 0.07), m['helmet'], parent=base)
    cube('v2_brow', canonical.xz(origin, 0.01, 1.74 + body_z, -0.37), (0.11, 0.05, 0.035), m['helmet'], rot=(0, 0.08, 0), parent=base)
    cube('v2_eye', canonical.xz(origin, -0.03, 1.70 + body_z, -0.39), (0.035, 0.04, 0.035), m['helmet'], parent=base)
    canonical.sphere('v2_badge_head', canonical.xz(origin, 0.19, 1.90 + body_z, -0.235), (0.040, 0.025, 0.040), m['ivory'], segments=10, rings=6, parent=base)
    cube('v2_badge_base', canonical.xz(origin, 0.19, 1.84 + body_z, -0.235), (0.11, 0.045, 0.035), m['ivory'], parent=base)
    weapon_z = 1.31 + body_z - crouch * 0.02
    rear_grip_x, support_x = add_weapon(weapon, origin, base, m, weapon_z)
    shoulder_rear = (0.06, 1.43 + body_z)
    shoulder_front = (0.00, 1.38 + body_z)
    rear_elbow = (-0.10, 1.30 + body_z)
    front_elbow = (-0.30, 1.24 + body_z)
    limb('v2_rear_upper_arm', origin, shoulder_rear, rear_elbow, 0.17, m['uniform'], base, y=-0.20)
    limb('v2_rear_forearm', origin, rear_elbow, (rear_grip_x, weapon_z - 0.03), 0.15, m['armor'], base, y=-0.33)
    limb('v2_front_upper_arm', origin, shoulder_front, front_elbow, 0.17, m['uniform2'], base, y=-0.24)
    limb('v2_front_forearm', origin, front_elbow, (support_x, weapon_z - 0.04), 0.15, m['armor'], base, y=-0.36)
...
def setup_scene():
    ...
    target = Vector(((canonical.COLS - 1) * CELL_WORLD / 2 + ORIGIN_SHIFT_X, 0.0, -(canonical.ROWS_PER_WEAPON - 1) * CELL_WORLD / 2 + CHARACTER_CENTER_Z))
    cam = scene.camera
    ...
    front.data.energy = 1850
    front.data.color = (1.0, 0.88, 0.76)
...
def main():
    ...
    for row, (action, count) in enumerate(canonical.ACTIONS):
        for frame in range(count):
            origin = (frame * CELL_WORLD + ORIGIN_SHIFT_X, 0.0, -row * CELL_WORLD)
