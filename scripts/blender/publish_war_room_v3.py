#!/usr/bin/env python3
"""Publish the generated War Room v3 shell to its isolated R2 channel."""
...
CANONICAL_PREFIX = "war-room/v3/runtime"
STAGING_ALIAS = "war-room/v3/staging/current.glb"
STAGING_REVISION_PREFIX = "war-room/v3/staging/revisions"
RUNTIME_ALIAS = "war-room/v3/runtime/current.glb"
RUNTIME_REVISION_PREFIX = "war-room/v3/runtime/revisions"
...
def main() -> int:
    ...
    if not source.is_file() or source.stat().st_size <= 60_000:
        raise SystemExit(f"War Room v3 GLB inválido: {source}")

    revision = str(args.revision or "").strip()
    if len(revision) < 7:
        raise SystemExit("War Room v3 revision inválida")
    ...
    if sha256_bytes(received) != digest:
        raise SystemExit(f"War Room v3 {args.channel} alias corrupto tras publicar")
    ...
    if sha256_bytes(received_revision) != digest:
        raise SystemExit("War Room v3 revision GLB corrupto tras publicar")

    print(
        f"War Room v3 R2 OK · revision={revision} · sha256={digest} · "
