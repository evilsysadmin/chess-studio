...
def main():
 ...
 manifest={'schema':1,'generation':'strict-v23','weapon':'machinegun','source_sha256':sha(a.source),'baseline_sha256':sha(a.baseline),'pistol_alignment_sha256':sha(a.pistol),'atlas_sha256':sha(a.output),'run13_sha256':sha(a.run_output),'atlas':{'size':list(SIZE),'cell_size':CELL,'columns':8,'rows':18,'pivot_x':200,'foot_y':382},'run13':{'size':[CELL*13,CELL],'frames':13,'cell_size':CELL,'fps':26.0},'status':status,'regenerated_rows':sorted(REGEN),'retained_reviewed_rows':[i for i in range(ROWS) if i not in REGEN],'notes':{'hurt':'6 authored phases + 2 intentional final-pose holds in fixed 8-column atlas','source':'dedicated transparent SMG technical source'}}
