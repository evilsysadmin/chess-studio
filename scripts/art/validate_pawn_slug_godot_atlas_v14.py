#!/usr/bin/env python3
"""Fail-closed validation for Matthias strict-v14 2D readability pass."""
...
def frame_hashes(atlas):
    return [
        hashlib.sha256(atlas[r * CELL:(r + 1) * CELL, c * CELL:(c + 1) * CELL].tobytes()).hexdigest()
        for r in range(ROWS)
        for c in range(COLS)
    ]
...
def main():
    ...
    if changed < 1_000_000:
        errors.append(f'readability pass too small: {changed} changed RGB pixels')
    ...
    for row in range(ROWS):
        ...
        if new_spread > old_spread + 1.0:
            errors.append(f'luma flicker regression row {row}: {old_spread:.2f}->{new_spread:.2f}')

    metrics = manifest.get('metrics', {})
    for key in ('skin_pixels', 'weapon_pixels', 'uniform_pixels', 'changed_rgb_pixels'):
        if int(metrics.get(key, 0)) <= 0:
            errors.append(f'missing metric {key}')
    if int(metrics.get('face_cue_frames', 0)) < 80:
        errors.append(f'face cue coverage too low: {metrics.get("face_cue_frames", 0)}')
    ...
    report = {
        ...
        'summary': {
            ...
            'face_cue_frames': int(metrics.get('face_cue_frames', 0)),
    ...
    if errors:
        raise SystemExit('\n'.join(errors))
    print('OK strict-v14 atlas', report['summary'])
