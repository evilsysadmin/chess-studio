...
UPPER_AREA_RATIO_MIN = 0.78
UPPER_AREA_RATIO_MAX = 1.28

IDLE_CORE_MAX_MIN_RATIO = 1.18
...
def validate_family(atlases: dict[str, Image.Image]) -> dict:
        ...
        if weapon != "pistol":
            for row, action in PARITY_ROWS.items():
                ...
                report["parity"][action] = {
                    ...
                    "upper_area_median_ratio": round(area_median, 4),
                }
                if not CORE_MEDIAN_RATIO_MIN <= core_median <= CORE_MEDIAN_RATIO_MAX:
                    errors.append(
                        f"{weapon}:{action}: core median ratio={core_median:.4f}"
                    )
                if (
                    min(core_ratios) < CORE_FRAME_RATIO_MIN
                    or max(core_ratios) > CORE_FRAME_RATIO_MAX
                ):
                    errors.append(
                        f"{weapon}:{action}: core frame range="
                        f"{min(core_ratios):.4f}..{max(core_ratios):.4f}"
                    )
                if not UPPER_WIDTH_RATIO_MIN <= width_median <= UPPER_WIDTH_RATIO_MAX:
                    errors.append(
                        f"{weapon}:{action}: upper width ratio={width_median:.4f}"
                    )
                if not UPPER_AREA_RATIO_MIN <= area_median <= UPPER_AREA_RATIO_MAX:
                    errors.append(
                        f"{weapon}:{action}: upper area ratio={area_median:.4f}"
                    )

            idle_values = [
                core_metrics(frame(atlas, 0, col))["area"]
                for col in range(COLS)
            ]
            idle_spread = max(idle_values) / min(idle_values)
            report["idle_core_max_min_ratio"] = round(idle_spread, 4)
            if idle_spread > IDLE_CORE_MAX_MIN_RATIO:
                errors.append(
                    f"{weapon}:idle volume jitter={idle_spread:.4f}"
                )
    ...
    return {
        ...
        "contract": {
            ...
            "idle_core_max_min_ratio": IDLE_CORE_MAX_MIN_RATIO,
