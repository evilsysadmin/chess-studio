...
# canónico del Makefile. Así ninguna prueba crítica se pierde ni aparece sólo en CI.
...
if sharded_playwright:
    ...
    lane_patterns = re.findall(r'--grep\s+"([^"]+)"', ci)
    if len(lane_patterns) != 2:
        raise SystemExit(f'CI shardado debe declarar exactamente dos greps críticos; encontrados: {len(lane_patterns)}')
    sharded_critical = {
        item.strip()
...
mode = 'lanes aisladas auditadas contra CRITICAL_E2E_GREP' if sharded_playwright else 'Make target e2e-critical'
