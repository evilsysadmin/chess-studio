...
def main() -> int:
    ...
    for token in (
        ...
        'staging : {service_name=\\"chess-studio-backend-staging\\"}',
        '{service_name=~\\"chess-studio-backend(-staging)?\\"}',
    ):
        if token not in logs_raw:
            fail(f"logs dashboard no usa OTLP canónico por entorno: {token}")
    ...
    for token in (
        ...
        '/api/folders',
        '/api/datasources',
        '_resolve_datasource_uid',
        'Datasource UID recovered',
        '/api/dashboards/db',
        '/api/dashboards/uid/',
    ...
    for token in (
        ...
        'oci_alloy_probe',
        'CHESS_STUDIO_ALLOY_PROBE_EMITTED',
        'backend-log-link-unavailable',
        'observability=${observability_summary:-unknown}',
        '--stability.level=public-preview',
