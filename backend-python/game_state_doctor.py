"""Read-only diagnostics for persisted Chess Studio game documents.

The doctor never talks to Mongo and never mutates a savegame. It accepts the
same raw document shape used by ``game_store`` and reconstructs the board via
the production ``chess_core`` contract. This makes it useful before/after a
manual restore without inventing a second rules implementation.
"""
...
from chess_core import load_board, serialize_game
