...
@pytest.mark.asyncio
async def test_daily_audience_is_reserved_once_and_failure_can_release_it():
...
@pytest.mark.asyncio
async def test_committed_daily_audience_cannot_be_spent_again():
    claim = await store.reserve("tester")
    result = await store.commit("tester", claim["reservation"], "action", "Tienes 8 partidas registradas. Haz una pausa antes de cada jugada crítica. Achtung: una tarea, no doce.")
