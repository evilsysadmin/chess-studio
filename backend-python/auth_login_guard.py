"""Shared login identity guard against distributed brute-force attempts."""
...
async def _get_collection():
    ...
    if persistent_storage_required():
        raise PersistentStorageUnavailable(
            "MongoDB no está disponible para el guard de login."
        )
    return None
...
async def _ensure_index(col) -> None:
    ...
    try:
        await col.create_index(
            ...
            name="auth_login_guard_ttl",
        )
    except PyMongoError as exc:
        raise PersistentStorageUnavailable(
            "No se pudo preparar el TTL del guard de login."
        ) from exc
    _index_ready = True
...
async def retry_after(identity: str) -> int:
    ...
    if col is not None:
        ...
        try:
            ...
            raise PersistentStorageUnavailable(
                "No se pudo consultar el guard de login."
            ) from exc
...
async def record_failure(identity: str) -> int:
    ...
    raise PersistentStorageUnavailable(
        "Demasiada contención al actualizar el guard de login."
    )


async def clear(identity: str) -> None:
    col = await _get_collection()
    if col is not None:
        await _ensure_index(col)
        try:
            await col.delete_one({"_id": identity})
        except PyMongoError as exc:
            raise PersistentStorageUnavailable(
                "No se pudo limpiar el guard de login."
            ) from exc
        return
    _memory.pop(identity, None)
