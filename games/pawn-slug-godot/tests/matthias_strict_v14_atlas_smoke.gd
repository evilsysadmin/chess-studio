...
func _init() -> void:
    ...
    if cols != 8 or rows != 18 or cell != 416:
        push_error("unexpected strict-v14 grid")
        quit(5)
        return
    ...
    sprite_frames.add_animation("strict_v14_smoke")
    for row in range(rows):
        for col in range(cols):
            ...
            frame.atlas = texture
            frame.region = region
            sprite_frames.add_frame("strict_v14_smoke", frame)
    if sprite_frames.get_frame_count("strict_v14_smoke") != 144:
        push_error("SpriteFrames did not consume all 144 cells")
        quit(8)
        return
    print("OK strict-v14 Godot atlas smoke: 144 AtlasTexture regions")
    quit(0)
