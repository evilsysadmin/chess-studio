...
func _initialize() -> void:
    ...
    if image.load(atlas_path) != OK:
        _fail("cannot load strict-v11 atlas")
        return
    var parsed = JSON.parse_string(FileAccess.get_file_as_string(manifest_path))
    if typeof(parsed) != TYPE_DICTIONARY:
        _fail("strict-v11 manifest is not an object")
        return
    var manifest: Dictionary = parsed
    if String(manifest.get("version", "")) != "v11":
        _fail("strict-v11 version drift")
        return
    ...
    if cols != 8 or rows != 18 or cell != 416:
        _fail("strict-v11 grid drift")
        return
    if image.get_size() != Vector2i(cols * cell, rows * cell):
        _fail("strict-v11 decoded size drift")
        return
    ...
    if actions.size() != rows:
        _fail("strict-v11 action count drift")
        return
    var total := 0
    for action_value in actions:
        if typeof(action_value) != TYPE_DICTIONARY:
            _fail("strict-v11 action entry invalid")
            return
        ...
        if row < 0 or row >= rows or count != 8:
            _fail("strict-v11 minimum-8 contract drift: " + name)
            return
        ...
        for col in range(count):
            ...
            total += 1
    if total != 144:
        _fail("strict-v11 total frame count drift")
        return
    ...
    for action_value in actions:
        ...
        if sprite_frames.get_frame_count(name) != 8:
            _fail("strict-v11 SpriteFrames drift: " + name)
            return
        sprite.play(name)
        sprite.frame = 7
        var frame_texture := sprite_frames.get_frame_texture(name, 7)
        if not (frame_texture is AtlasTexture):
            _fail("strict-v11 frame is not AtlasTexture: " + name)
            return
    print("OK Godot strict-v11 runtime smoke: 18 poses x 8 frames = 144")
    quit(0)
