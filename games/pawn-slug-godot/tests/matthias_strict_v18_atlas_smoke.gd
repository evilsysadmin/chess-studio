...
func _init() -> void:
    ...
    if typeof(manifest) != TYPE_DICTIONARY:
        ...
        quit(4)
        return
    if String(manifest.get("version", "")) != "v18" or String(manifest.get("weapon", "")) != "pistol":
        push_error("unexpected strict-v18 identity")
        quit(5)
        return
    ...
    if cols != 8 or rows != 18 or cell != 416:
        push_error("unexpected strict-v18 grid")
        quit(6)
        return
    ...
    sprite_frames.add_animation("strict_v18_smoke")
    for row in range(rows):
        for col in range(cols):
            ...
            frame.atlas = texture
            frame.region = region
            sprite_frames.add_frame("strict_v18_smoke", frame)
    if sprite_frames.get_frame_count("strict_v18_smoke") != 144:
        push_error("SpriteFrames did not consume all 144 cells")
        quit(9)
        return
    print("OK strict-v18 Godot atlas smoke: 144 AtlasTexture regions")
    quit(0)
