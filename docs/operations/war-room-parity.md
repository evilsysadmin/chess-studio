...
| Teclado | **GATE** | `three-d-war-room.spec.js`: foco roving equivalente y movimiento por flechas + Enter. | Cubrir especiales y orientación negra. |
| Highlights selección/targets/captura/jaque | **CUBIERTO** | `Board3DHighlights.test.js` define precedencias y colores; la escena 3D dibuja estados físicos sobre la losa. | Acreditar captura normal y especiales durante flujo cross-renderer. |
...
| En passant | **CUBIERTO** | Backend/rules protegen legalidad y `WarRoom3DMotion.test.js` acredita inferencia de la pieza capturada fuera de la casilla destino. | E2E 2D↔3D que verifique peón retirado, FEN final y una sola mutación. |
...
1. Añadir gates cross-renderer de **enroque**, **en passant** y **promoción**, en slices separados y deterministas.
2. Añadir captura normal si no queda subsumida de forma suficientemente clara por en passant.
3. Añadir torture test **2D→3D→2D→3D**, incluyendo selección/cancelación y un movimiento final único.
