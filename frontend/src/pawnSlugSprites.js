...
function atlasSprite(primaryUrl, fallbackUrl, frames, initialFrame = 0, scale = [2.2, 2.2]) {
  ...
  sprite.userData.atlas = { frames, frame: initialFrame, texture: null, source: 'primary' };

  const loader = new THREE.TextureLoader();

  function swapTexture(texture, source) {
    const previous = sprite.userData.atlas.texture;
    configureAtlasWindow(configureTexture(texture), frames, sprite.userData.atlas.frame);
    sprite.userData.atlas.texture = texture;
    sprite.userData.atlas.source = source;
    material.map = texture;
    material.needsUpdate = true;
    if (previous && previous !== texture) previous.dispose?.();
  }

  function loadTexture(url, source) {
    const texture = loader.load(
      url,
      (loaded) => {
        if (sprite.userData.atlas.texture === loaded) {
          configureAtlasWindow(configureTexture(loaded), frames, sprite.userData.atlas.frame);
          material.needsUpdate = true;
        }
      },
      undefined,
      () => {
        if (source === 'primary' && fallbackUrl && fallbackUrl !== primaryUrl) {
          loadTexture(fallbackUrl, 'fallback');
        }
      },
    );
    swapTexture(texture, source);
    return texture;
  }

  loadTexture(primaryUrl, 'primary');
  ...
  sprite.userData.setFrame = (frame) => {
    ...
    if (texture) texture.offset.x = next / frames;
  };
  return sprite;
}
