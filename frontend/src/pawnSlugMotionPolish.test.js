...
describe('Pawn Slug locomotion polish', () => {
  it('keeps only a brief walk lead-in before the dedicated run cycle owns locomotion', () => {
    expect(pawnSlugMatthiasLocomotion({ time: 10.04, moving: true, speedRatio: 0.2, moveStartedAt: 10 })).toMatchObject({ action: 'walk', phase: 'walk' });
    expect(pawnSlugMatthiasLocomotion({ time: 10.1, moving: true, speedRatio: 0.6, moveStartedAt: 10 })).toMatchObject({ action: 'run', phase: 'run' });
    expect(PAWN_SLUG_MOTION_POLISH.runSpeedThreshold).toBeLessThanOrEqual(0.35);
    expect(PAWN_SLUG_MOTION_POLISH.walkToRunSeconds).toBeLessThanOrEqual(0.1);
  });
  ...
  it('slows the walk cycle while Matthias is still accelerating', () => {
    const slow = pawnSlugMatthiasLocomotion({ time: 12.06, moving: true, speedRatio: 0.12, moveStartedAt: 12 });
    const brisk = pawnSlugMatthiasLocomotion({ time: 12.06, moving: true, speedRatio: 0.3, moveStartedAt: 12 });
  ...
  it('matches the approved v5 walk cadence while keeping the stop settle compact', () => {
    expect(PAWN_SLUG_MOTION_POLISH.walkFrames).toBe(10);
    expect(PAWN_SLUG_MOTION_POLISH.walkRate).toBeGreaterThan(9);
    expect(PAWN_SLUG_MOTION_POLISH.settleFrames).toBeGreaterThanOrEqual(4);
    expect(PAWN_SLUG_MOTION_POLISH.settleSeconds).toBeLessThanOrEqual(0.08);
  });
