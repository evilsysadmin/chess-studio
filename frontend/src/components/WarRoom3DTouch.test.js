...
import { CLASSIC_DESKTOP_PIECE_HIT_TARGET, COARSE_PIECE_HIT_TARGET, resolveBoardTap } from './WarRoom3DTouch.js';
...
describe('War Room 3D touch targeting', () => {
  ...
  it('mantiene los hit targets pegados a la peana para no robar la fila vecina en perspectiva', () => {
    ...
    expect(CLASSIC_DESKTOP_PIECE_HIT_TARGET.radius).toBeLessThan(COARSE_PIECE_HIT_TARGET.radius);
    expect(CLASSIC_DESKTOP_PIECE_HIT_TARGET.height).toBeLessThan(COARSE_PIECE_HIT_TARGET.height);
    expect(CLASSIC_DESKTOP_PIECE_HIT_TARGET.centerY).toBeLessThan(0.25);
  });
});
