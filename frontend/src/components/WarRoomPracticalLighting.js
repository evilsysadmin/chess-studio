...
export function tuneWarRoomGalleryTorchWallWash(group) {
  ...
  for (const side of ['left', 'right']) {
    ...
    if (needsFlameFinish) {
      // Keep the bright geometry, but use much deeper emissive hues. The old
      // orange/yellow emissives were multiplied by high intensities until both
      // red and green channels clipped, which made the flame read pale/white.
    ...
    // Let the light's existing organic flicker drive a restrained emissive
    // pulse too. This makes the fire itself breathe without adding another RAF
    // loop or changing the already-approved wall illumination.
