...
describe('HomeBlenderScene3D hearth animation', () => {
  it('classifies only authored hearth meshes as animated fire', () => {
    ...
    expect(homeBlenderFireKind('HOME_PROP_chandelier_flame_0')).toBeNull();
    expect(homeBlenderFireKind('HOME_PROP_fireplace_left_log_a')).toBeNull();
  });

  it('keeps deterministic fire motion restrained around the authored shape', () => {
    for (const kind of ['flame', 'hot', 'ember']) {
