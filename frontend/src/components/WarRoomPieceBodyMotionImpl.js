...
export function resetPieceBodyMotion(group) {
  const reset = group?.userData?.board3DResetBodyMotion;
...
export function installPieceBodyMotion(group, type, { coarsePointer = false, reducedMotion = null } = {}) {
  ...
  function resetPose({ invalidateTravel = false } = {}) {
    const hadTransientState = Boolean(
      state.active
      || state.finishEvent
      || state.passiveCheckEvent
      || state.promotionGhost
      || state.promotionMaterialStates
      || state.maxDistance > EPSILON
      || (invalidateTravel && state.targetSquare),
    );
    ...
    return hadTransientState || morphChanged || passiveChanged;
  }

  function hardReset() {
    return resetPose({ invalidateTravel: true });
  }
  ...
  group.userData.board3DResetBodyMotion = hardReset;
  group.userData.board3DBodyInterruptProfile = 'piece-body-hard-reset-v1';
