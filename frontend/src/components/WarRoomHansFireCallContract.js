...
export const HANS_INITIAL_REPLY_MAX_ABS_NDC_X = 0.72;
...
export function hansInitialReplyPointReached({ hansScreen = 'missing', ndcX } = {}) {
  const x = Number(ndcX);
  return hansScreen === 'onscreen'
    && Number.isFinite(x)
    && Math.abs(x) <= HANS_INITIAL_REPLY_MAX_ABS_NDC_X;
}
