...
export default function SpectatorScreen({ onExit }) {
  ...
  async function runMatchLoop(wLevel, bLevel, generation, controller) {
    ...
    const stale = () => stopRef.current || controller.signal.aborted || generationRef.current !== generation;
    ...
    await runSpectatorSession({
      ...
      isCurrentPosition: (chess, requestedFen) => chessRef.current === chess && chess.fen() === requestedFen,
      analyzePosition: (requestedFen, level, signal) => api.analyzePosition(requestedFen, level, { signal }),
      ...
      onError: (error) => {
        setError(error?.message || 'La partida espectador se interrumpió.');
        setPhase('setup');
      },
    });

    if (generationRef.current === generation && loopAbortRef.current === controller) loopAbortRef.current = null;
  }
