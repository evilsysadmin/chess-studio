...
export default function CareerScreen({ history, ratingHistory, onExit, onOpenRecord, onMovie, onPlayFromHere, onOpenPuzzles, onStartRun, onContinueRun, embedded = false }) {
  ...
  return <div className={embedded ? 'career-screen insights-embedded-page' : 'menu tournament-panel career-screen'}>
    ...
    <div className="menu-section"><h2>📅 Informe semanal</h2><div className="career-mini-grid"><span><b>{weekly.current.games}</b><small>partidas últimos 7 días</small></span><span><b>{weekly.current.winPct}%</b><small>victorias · {weekly.winDelta===null?'sin comparación':`${weekly.winDelta>=0?'+':''}${weekly.winDelta} pts vs semana anterior`}</small></span><span><b>{pct(weekly.current.accuracy)}</b><small>accuracy media {weekly.accuracyDelta===null?'':`· ${weekly.accuracyDelta>=0?'+':''}${weekly.accuracyDelta} vs anterior`}</small></span><span><b>{topIncident?`${topIncident.count}×`:'—'}</b><small>{topIncident?.label||'sin pecado dominante registrado'}</small></span></div><p className="hint-text">{weekly.current.games===0?'Semana sospechosamente silenciosa. El tablero no puede juzgar lo que no ve.':weekly.winDelta!==null&&weekly.winDelta>10?'Mejora clara. Molesto para el departamento de sarcasmo; excelente para ti.':weekly.winDelta!==null&&weekly.winDelta<-10?'La semana ha venido con pendientes. Conviene revisar autopsias antes de convertir la tendencia en tradición.':'Semana estable. Sigue acumulando datos antes de declarar una revolución ajedrecística.'}</p></div>
    ...
    <div className="menu-section"><h2>☠ Hall of Shame</h2><div className="museum-grid"><div className="museum-card"><span>💥</span><strong>{hall.worst?`−${hall.worst.worst.loss} cp`:'—'}</strong><small>peor blunder analizado</small></div><div className="museum-card"><span>📉</span><strong>{hall.missedConversion?`+${(hall.missedConversion.peakPerspectiveEval/100).toFixed(1)}`:'—'}</strong><small>mayor ventaja no convertida</small></div><div className="museum-card"><span>🏚</span><strong>{donations.queens}</strong><small>damas entregadas a la contabilidad rival</small></div><div className="museum-card"><span>🧾</span><strong>{recurrence.repeated}</strong><small>errores reincidentes</small></div></div></div>
