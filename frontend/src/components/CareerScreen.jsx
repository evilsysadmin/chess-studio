...
export default function CareerScreen({ history, ratingHistory, onExit, onOpenRecord, onMovie, onPlayFromHere, onOpenPuzzles, onStartRun, onContinueRun, embedded = false }) {
  ...
  const [careerSection,setCareerSection]=useState('summary');
  ...
  return <div className={embedded ? 'career-screen insights-embedded-page' : 'menu tournament-panel career-screen'}>
    ...
    <nav className="career-section-nav" aria-label="Secciones de Mi progreso">
      {[['summary','Resumen'],['performance','Rendimiento'],['openings','Aperturas'],['archive','Archivo']].map(([id,label])=><button type="button" key={id} className={careerSection===id?'active':''} aria-current={careerSection===id?'page':undefined} onClick={()=>setCareerSection(id)}>{label}</button>)}
    </nav>
      ...
      {nemesis.opening ? <>
        ...
        <div className="nemesis-card">
          <div className="nemesis-core">
            ...
            <small>CPU · nivel medio {nemesis.opening.avgDifficulty} · últimas {Math.min(5,nemesis.opening.games)}: {nemesis.opening.recentScorePct}%.</small>
          </div>
    ...
    <div className="menu-section career-rpg-section"><div className="career-rpg-heading"><div><span className="section-label">Perfil RPG basado en datos</span><h2>🧬 {rpgSummary.title}</h2></div><small>{rpgSummary.games} partidas registradas · sólo datos medidos.</small></div><div className="career-rpg-summary-grid"><article><span>Identidad</span><b>{rpgSummary.title}</b><small>{rpgSummary.measuredCount} atributos con muestra real.</small></article><article><span>Fortaleza medida</span><b>{rpgSummary.leader?`${rpgSummary.leader.label} · ${rpgSummary.leader.value}%`:'Aún sin muestra'}</b><small>{rpgSummary.leader?.sample?`${rpgSummary.leader.sample} muestras.`:'Juega y analiza más partidas para que aparezca.'}</small></article><article><span>Más bajo medido</span><b>{rpgSummary.lowest?`${rpgSummary.lowest.label} · ${rpgSummary.lowest.value}%`:'Aún sin contraste'}</b><small>{rpgSummary.lowest?.sample?`${rpgSummary.lowest.sample} muestras · no es un diagnóstico.`:'Hace falta más de un atributo medible.'}</small></article></div><details className="friendly-disclosure career-rpg-details"><summary>Ver atributos y método</summary><div className="friendly-disclosure-body"><div className="career-rpg-grid">{rpg.attributes.map(a=><article className={`career-rpg-stat ${a.id===rpg.leaderId?'leader':''}`} key={a.id} title={a.explanation}><span>{a.label}</span><b>{a.value===null?'—':`${a.value}%`}</b><div><i style={{width:`${a.value||0}%`}}/></div><small>{a.sample?`${a.sample} muestras · ${a.explanation}`:'Sin muestras suficientes.'}</small></article>)}</div><p className="hint-text">Estos índices resumen datos ya registrados. No son ELO, talento oculto ni una evaluación clínica de tu dignidad ajedrecística.</p></div></details></div>
    ...
    {careerSection==='openings'&&<>
    {openings.length>0&&<div className="menu-section"><h2>⚔ Rivalidad por aperturas</h2><div className="career-rhythm-grid">{openings.slice(0,8).map(row=><div className="career-rhythm" key={row.opening}><b>{row.opening}</b><span>{row.wins}V · {row.draws}T · {row.losses}D · {row.scorePct}%</span><small>{row.games} partidas · CPU · nivel medio {row.avgDifficulty}</small></div>)}</div>{clinic.length>0&&<><h3>🩺 Clínica de aperturas</h3><p className="hint-text">Aperturas con al menos 3 muestras y rendimiento inferior al 50%. Aquí sí hay prueba, no horóscopo.</p><div className="career-training-list">{clinic.map(row=><div key={row.opening}><b>{row.opening} · {row.scorePct}%</b><p>Revisa tus primeras 10–12 jugadas de estas {row.games} partidas y compara el primer punto donde sales de tus líneas habituales. El nivel medio de CPU era {row.avgDifficulty}.</p></div>)}</div></>}</div>}
    {tree.count>0&&<div className="menu-section career-opening-map"><div className="career-section-heading"><div><span className="section-label">PRIMERAS DECISIONES</span><h2>Mapa de aperturas</h2></div><small>Top 5 líneas · hasta 3 jugadas</small></div><div className="career-tree"><TreeRows node={tree}/></div></div>}
    </>}
