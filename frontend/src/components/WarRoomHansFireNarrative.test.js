...
function makeHarness() {
  const fireCore = { name: 'war-room-fire-core', visible: true, scale: makeScale(1, 1, 1) };
  const fireLight = { name: 'war-room-fire-light', intensity: 4, distance: 10, userData: { baseWarRoomIntensity: 4 } };
  const bounce = { name: 'war-room-fire-bounce-light', intensity: 2, userData: { hansBaseIntensity: 2 } };
  const fireplace = {
  ...
  const driver = {
    ...
    userData: { warRoomHansQuickIteration: 'forced-test', warRoomHansPhase: 'fire-dimming' },
    onBeforeRender() {},
  };
  const root = {
    getObjectByName(name) {
