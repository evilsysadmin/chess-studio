...
export function matthiasPuppetGestureDelay({ speaking = false, random = Math.random } = {}) {
  if (speaking) return 120;
  const sample = Math.min(1, Math.max(0, Number(random?.()) || 0));
  return Math.round(QUIET_MIN_DELAY_MS + (sample * QUIET_DELAY_SPREAD_MS));
}
...
export function matthiasPuppetGesturePlan(kind = 'acknowledge') {
  ...
  if (kind === 'sip') {
    return {
      ...
      head: [
        ...
        { offset: .45, transform: 'rotate(-4deg)' },
        { offset: .72, transform: 'rotate(-3deg)' },
        { offset: 1, transform: 'rotate(0deg)' },
      ],
      actionArm: [
        { offset: 0, transform: 'rotate(0deg)' },
        { offset: .42, transform: 'rotate(-31deg)' },
        { offset: .72, transform: 'rotate(-29deg)' },
        { offset: 1, transform: 'rotate(0deg)' },
      ],
      eyes: [
  ...
  if (kind === 'bite') {
    return {
      ...
      head: [
        { offset: 0, transform: 'rotate(0deg)' },
        { offset: .42, transform: 'rotate(3deg)' },
        { offset: .62, transform: 'rotate(1deg)' },
        { offset: 1, transform: 'rotate(0deg)' },
      ],
      actionArm: [
        { offset: 0, transform: 'rotate(0deg)' },
        { offset: .4, transform: 'rotate(-25deg)' },
        { offset: .66, transform: 'rotate(-22deg)' },
        { offset: 1, transform: 'rotate(0deg)' },
      ],
    };
  }
  ...
  if (kind === 'read') {
    return {
      ...
      head: [
        { offset: 0, transform: 'rotate(0deg)' },
        { offset: .38, transform: 'rotate(4.5deg)' },
        { offset: .72, transform: 'rotate(3.5deg)' },
        { offset: 1, transform: 'rotate(0deg)' },
      ],
      eyes: [
  ...
  if (kind === 'inspect') {
    return {
      ...
      head: [
        { offset: 0, transform: 'rotate(0deg)' },
        { offset: .38, transform: 'rotate(-5deg)' },
        { offset: .7, transform: 'rotate(-3deg)' },
        { offset: 1, transform: 'rotate(0deg)' },
      ],
      actionArm: [
        { transform: 'rotate(0deg)' },
...
function animatePart(node, frames, duration, options = {}) {
...
export default function MatthiasPuppet({ scene = 'base', speaking = false, reducedMotion = false }) {
  ...
  useEffect(() => {
    ...
    function schedule(delay) {
      timeout = window.setTimeout(runGesture, delay);
    }
    ...
    schedule(matthiasPuppetGestureDelay({ speaking }));
  ...
  return (
    ...
    >
      ...
      <g
        ref={actionArmRef}
        className="matthias-puppet__arm matthias-puppet__arm--action"
        data-puppet-part="action-arm"
      >
