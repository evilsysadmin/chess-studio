...
// Forecast of what a party move will cost, computed with the exact pipeline the
// runtime uses (chroniclesTacticsMove -> chroniclesTacticsResolvePlayerAction):
// the creature response only happens when combat is active before or after the
// move, and it is the same pure resolver, so the forecast cannot drift from play.
...
// Forecasts keyed by move key (north/east/south/west) for every legal move.
...
// Short text shown on the move button; empty when nothing is threatened.
...
// Full sentence for assistive tech and tooltips; empty when nothing is threatened.
