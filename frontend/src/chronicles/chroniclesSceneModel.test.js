...
describe('Chronicles scene model projection', () => {
  ...
  it('projects every shipped map through the same renderer contract', () => {
    chroniclesMapIds().forEach((mapId) => {
      ...
      expect(model.focusCell.x).toBeGreaterThanOrEqual(0);
      expect(model.focusCell.y).toBeGreaterThanOrEqual(0);
  ...
  it('fails closed when an alternate renderer receives a malformed scene model', () => {
    ...
    expect(() => chroniclesValidateSceneModel({ ...valid, focusCell: { x: Number.NaN, y: 1 } })).toThrow(/focusCell/i);
