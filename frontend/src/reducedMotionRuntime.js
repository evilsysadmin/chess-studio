...
export const REDUCED_MOTION_RUNTIME_KEY = 'chess-study-reduced-motion';
...
function installInvalidationListeners() {
  ...
  window.addEventListener('storage', (event) => {
    if (event.key === null || event.key === REDUCED_MOTION_RUNTIME_KEY) invalidate();
  });
}

export function getEffectiveReducedMotionRuntime() {
  installInvalidationListeners();
  if (cachedEffective !== undefined) return cachedEffective;

  const stored = getStorageItem(STORAGE_LOCAL, REDUCED_MOTION_RUNTIME_KEY);
