...
describe('Chronicles Tactics · class doctrine skills', () => {
  ...
  it('unlocks one level-4 spell branch and makes it change the real ability', () => {
    ...
    expect(competing.unlocked).toBe(false);
    expect(competing.reason).toMatch(/rama|doctrina/i);

    const woundedParty = createChroniclesState().party.map((member) => ({ ...member, hp: Math.max(1, member.hp - 4) }));
