...
describe('Chronicles Tactics · class doctrine skills', () => {
  ...
  it('opens a second Aziz grimoire page at level 6', () => {
    ...
    const blocked = unlockChroniclesSkill(solar.progression, 'bishop', 'bishop-aurora-liturgy');
    ...
    const healed = chroniclesTacticsAbility(healState, 'bishop');
    expect(healed.party.find((member) => member.id === 'matthias').hp).toBe(6);
  });
