...
function material(color, roughness = 0.8, metalness = 0.08) {
...
function commandPost(x, coarse) {
  ...
  if (!coarse) {
    const lamp = mesh(new THREE.SphereGeometry(0.09, 8, 6), new THREE.MeshBasicMaterial({ color: 0xe7b86a }), { x: 1.26, y: 0.82, z: 0.87 });
...
function wreckedSearchlight(x, coarse) {
  ...
  const lens = mesh(new THREE.CircleGeometry(0.31, coarse ? 10 : 18), new THREE.MeshBasicMaterial({ color: 0x8c744e, transparent: true, opacity: 0.55 }), { x: 0.27, ry: Math.PI / 2 });
...
function heroBarricade(x, coarse) {
  ...
  if (!coarse) {
    root.add(
      mesh(new THREE.BoxGeometry(0.08, 0.36, 0.04), new THREE.MeshBasicMaterial({ color: 0xb64934 }), { x: -0.5, y: 0.95, z: 0.075 }),
      mesh(new THREE.BoxGeometry(0.36, 0.08, 0.04), new THREE.MeshBasicMaterial({ color: 0xb64934 }), { x: -0.5, y: 0.95, z: 0.077 }),
