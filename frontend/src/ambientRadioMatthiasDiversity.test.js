...
import {
  RADIO_MATTHIAS_MELODIC_REWRITE_IDS,
  RADIO_MATTHIAS_MELODIC_REWRITES,
} from './ambientRadioMatthiasRecompositions.js';
...
describe('Radio Matthias · diversidad melódica', () => {
  it('instala las siete recomposiciones sin cambiar sus ids públicos', () => {
    expect(RADIO_MATTHIAS_MELODIC_REWRITE_IDS).toHaveLength(7);
    for (const id of RADIO_MATTHIAS_MELODIC_REWRITE_IDS) {
  ...
  it('da a cada recomposición una ruta armónica propia', () => {
    const paths = RADIO_MATTHIAS_MELODIC_REWRITE_IDS.map((id) => (
      JSON.stringify(structuredFeel(AMBIENT_THEMES[id]).harmonyPath)
    ));
    expect(new Set(paths).size).toBe(paths.length);
  });

  it('evita que dos temas vuelvan a compartir el mismo esqueleto de intervalos', () => {
    const fingerprints = RADIO_MATTHIAS_MELODIC_REWRITE_IDS.map((id) => ({
      id,
