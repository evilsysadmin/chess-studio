...
describe('orchestral sampler', () => {
  ...
  it('ships the complete compact sample map under a two-megabyte budget', () => {
    const files = Object.values(ORCHESTRAL_SAMPLE_LIBRARY).flat().map(({ file }) => (
      path.resolve(process.cwd(), 'public/audio/orchestra', file)
    ));
    files.forEach((file) => expect(fs.statSync(file).size).toBeGreaterThan(20_000));
