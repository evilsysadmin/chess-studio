...
# Production frontend DNS is owned by scripts/cloudflare_production_pages.py.
# Production API DNS is owned by scripts/oci_production_tunnel.py so the
# backend target can switch atomically between Render and OCI after exact-SHA
# readiness. Terraform deliberately owns neither public application CNAME.
