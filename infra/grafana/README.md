...
`.github/workflows/billing-cost-export.yml` ejecuta `scripts/billing_cost_export.py` cada seis horas. Reutiliza las credenciales OCI ya presentes en Actions, consulta OCI Usage API con `query_type=COST`, consulta el endpoint de Cloudflare Billing `/accounts/<id>/billable-usage` y publica ambas cifras por el mismo OTLP de Grafana que usa staging.
